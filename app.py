...
def show_main_page():
    ...
    if analyze_now and raw_text.strip():
        with st.spinner("Analyzing your text... 🧠"):
            ...
            if analysis_mode == "Emotion Detection":
                ...
                st.subheader("🎧 Music & Quote Recommendation")
                suggestion = get_music_suggestion(prediction)
                st.write(f"**🎶 {suggestion['title']}**")
                st.markdown(f"[▶️ Listen here]({suggestion['url']})")
                st.caption(suggestion['description'])

                current_hour = datetime.datetime.now().hour
                if current_hour < 12:
                    st.info("☀️ Morning vibes: Start your day with calm focus.")
                elif current_hour < 18:
                    st.info("🌞 Afternoon boost: Keep going, you're doing great!")
                else:
                    st.info("🌙 Evening mood: Relax and reflect.")

                quotes = {
                    "joy": "Happiness is not something ready-made. It comes from your own actions. – Dalai Lama",
                    "sadness": "Tears come from the heart and not from the brain. – Leonardo da Vinci",
                    ...
                    "surprise": "Surprise is the greatest gift which life can grant us. – Boris Pasternak",
                    "positive": "Keep your face always toward the sunshine—and shadows will fall behind you. – Walt Whitman",
                    "negative": "Even the darkest night will end and the sun will rise. – Victor Hugo",
                    "neutral": "Life is a balance of holding on and letting go."
                }
                st.markdown(f"💬 **Quote:** _{quotes.get(prediction.lower(), 'Stay positive!')}_")
                ...
                quotes = {
                    "positive": "The best way to predict your future is to create it. – Abraham Lincoln",
                    "negative": "It is during our darkest moments that we must focus to see the light. – Aristotle",
                    "neutral": "The art of life is a constant readjustment to our surroundings. – Kakuzo Okakura"
                }
                st.markdown(f"💬 **Quote:** _{quotes.get(prediction.lower(), 'Keep going, stay strong!')}_")
        ...
        with st.expander("☁️ Word Cloud from Input Text"):
        ...
        with open(pdf_path, "rb") as f:
            ...
            st.download_button(label="📄 📅️ Download PDF Report", data=f, file_name=file_name, mime="application/pdf")
    ...
    if "predicted_emotion" in st.session_state:
        with st.expander("💬 Chat with TonoSense Bot"):
    ...
    st.markdown("<div style='text-align: center; color: gray;'>Built with bugs, fixed with tears, shipped with love 💔➡️❤️ | Powered by Minark’s overthinking 🤔🤯💡</div>", unsafe_allow_html=True)
